...
Eso representaria el retorno de la inversión. Por ejemplo, si presupuesto fue de 1 millon y se ganaron 3 tendríamos un ROI del 200%.
...
Algunas películas habrían no tendrían una ganacia asignada ya que la operación habría arrojado un (NaN), porque no es posible calcular una resta cuando falta uno de los operadores.
...
Drama, no tenía ninguna espectativa con el resultado por lo que tampoco me generó sorpresa.
...
Verificar la consistencia y el formato para poder trabajarlos uniformemente. Si algunas fechas estuvieran como texto o en formatos diferentes, primero las convertiría garantizar que las operaciones arrojen el resultado esperado.
...
Se mantendrían igual las operaciones de carga y guardado. Lo que cambiaría sería las operaciones realizadas con columnas especificas ya que cambiaría el proceso porque el contexto no es el mismo. Por ejemplo la columna exitosa, se debe considerar si se define por ventas o por reproducciones.
...
Evaluaría si realmente aporta valor al análisis. En este caso la eliminarla porque hay muy poca información relevante.